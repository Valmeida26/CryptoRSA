package com.example.cryptoappexample;
...
public class RSAcrypt {
    ...
    public static String rsaText(String message){
        ...
        try {
            if (message == null || message.isEmpty()) {
                return "Enter a word to encrypt.";
            }
                encrypt = encryptRSA(publicKey, message);
                String dataEncrypt = null;
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                    dataEncrypt = Base64.getEncoder().encodeToString(encrypt);
                }

                byte[] decrypt = new byte[0];
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                    decrypt = decryptRSA(privateKey, dataEncrypt);
                }
            return "ENCRYPTED: \n" + dataEncrypt + "\n\n\nDECRYPTED: \n" + new String(decrypt, StandardCharsets.UTF_8);
        } catch (Exception e) {
            return "Error processing encryption:" + e.getMessage();
        }
    }
...
//        byte[] publicKeyBytes = Files.readAllBytes(Paths.get(filePathPublic));
        ...
        try {
            ...
            throw new IOException("Error reading public key: " + e.getMessage());
        }
    }
...
//        byte[] privateKeyBytes = Files.readAllBytes(Paths.get(filePathPrivate));
        ...
        try {
            ...
            throw new IOException("Error reading private key: " + e.getMessage());
        }
    }
